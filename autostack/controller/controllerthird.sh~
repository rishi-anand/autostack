...
sudo cp ~/autostack/controller/keystone.conf /etc/keystone/ || check=false
echo -------------------$filename line no : $linenumber------------------------
#line no 2
fi
...
sudo cp ~/autostack/controller/admin-openrc.sh ~/ || check=false
sudo cp ~/autostack/controller/demo-openrc.sh ~/ || check=false
...
sudo cp ~/autostack/controller/glance-api.conf /etc/glance/ || check=false
...
sudo cp ~/autostack/controller/glance-registry.conf /etc/glance/ || check=false
...
sudo cp ~/autostack/controller/nova.conf /etc/nova/ || check=false
...
sshpass -p welcome ssh -o StrictHostKeyChecking=no cliqr@192.168.4.204 "sudo -u root  ~/autostack/compute/computenova.sh" || check=false
...
echo --- if- you PRESS n, then- execute ~/autostack/controller/extra_sh/controllerrest.sh to execute rest of script ------
...
sudo cp ~/autostack/controller/ml2_conf.ini /etc/neutron/plugins/ml2/ || check=false
...
sudo cp ~/autostack/controller/network_nova/nova.conf /etc/nova/ || check=false
...
sshpass -p welcome ssh -o StrictHostKeyChecking=no cliqr@192.168.4.202 "sudo -u root  ~/autostack/network/networknetwork.sh" || check=false
...
if [ "$check" = true ] ; then
((linenumber=linenumber+1))
sudo cp ~/autostack/controller/network_nova/netwotk_network_nova/nova.conf /etc/nova/ || check=false
echo -------------------$filename line no : $linenumber------------------------
#line no 85
fi
...
sshpass -p welcome ssh -o StrictHostKeyChecking=no cliqr@192.168.4.202 "sudo -u root  ~/autostack/network/networknetworksecond.sh" || check=false
...
echo --- Going To Compute Node ---
sshpass -p welcome ssh -o StrictHostKeyChecking=no cliqr@192.168.4.204 "sudo -u root  ~/autostack/compute/computenetwork.sh" || check=false
...
sudo cp ~/autostack/controller/local_settings.py /etc/openstack-dashboard/ || check=false
